					...
					<Table className={`table-striped table-bordered table-hover mb-9`} responsive>
						<thead>
							<tr>
								...
								<th className="text-nowrap text-right fs-5">Chiết khấu</th>
								<th className="text-nowrap text-center fs-5">Trạng thái</th>
								<th className="text-nowrap text-center fs-5">Loại thanh toán</th>
								<th className="text-nowrap text-center fs-5">Trạng thái thanh toán</th>
								<th className="text-nowrap text-center fs-5">Trạng thái giao hàng</th>
								...
								props.orders?.length > 0 && props.orders.map((item, key) => {
									return (
										< tr key={key} className="table-product">
											<td>{item.id}</td>
											<td>
												<span className="font-weight-bold">Tên người dùng:</span><span> {item.receiver_name} <br /></span>
												<span className="font-weight-bold">Số điện thoại:</span><span> {item.receiver_phone} <br /></span>
											</td>
